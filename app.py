...
def run_s5p_site_scoring(df, radius_km, days_recent, days_baseline):
    ...
    try:
        ...
        return out, f"S5P Analysis Completed successfully for latest image ending {latest_dt.strftime('%Y-%m-%d')}."
    except Exception as exc:
        return None, f"Earth Engine execution error: {str(exc)}"
...
def calculate_physics_and_risk(df):
    ...
    # 1. Atmospheric Transport / Gaussian Plume Screening Advection Math
    # Distance (km) = [ Wind Speed (m/s) * 1800 sec (30 mins) ] / 1000
    ...
    # 2. Synthetic Pressure & Surface Gas Migration Screening Proxy
    data["effective_ch4"] = data["methane_ppb"].apply(lambda x: safe_num(x, default=1850.0))
    data["subsurface_pressure_proxy_psi"] = (
        data["pressure_psi"].fillna(data["effective_ch4"] * 0.01)
    )

    # 3. Composite Risk Engine Normalization
    ch4_norm = normalize_0_100(data["ef