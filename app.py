...
st.sidebar.markdown("---")
if EE_OK:
    if st.sidebar.button("🛰️ Check latest S5P observation", use_container_width=True):
        with st.spinner("Checking latest Sentinel-5P metadata..."):
            st.session_state.latest_s5p = ee_s5p_latest_info()
...
def mohr_coulomb_shear_strength(cohesion_kpa, effective_stress_kpa, friction_angle_deg):
    """ Mohr-Coulomb: tau = c + sigma' tan(phi) """
...
def gaussian_plume_centerline(q_kg_s, wind_m_s, sigma_y_m, sigma_z_m):
    """ Simplified Gaussian plume centerline concentration factor. Not a regulatory atmospheric model. """
...
def spatial_cluster_index(df, k=6):
    """ Lightweight inverse-distance clustering index. It is a screening statistic, not a full GIS Moran's I implementation. """
    if len(df) < 3:
...
latest_label = (
    st.session_state.latest_s5p["text"]
    if st.session_state.latest_s5p is not None
...
st.markdown(
    f""" <div class="card"> <b>Data mode:</b> {data_source} &nbsp;&nbsp; | &nbsp;&nbsp; <b>Earth Engine:</b> {"CONNECTED" if EE_OK else "OFFLINE"} &nbsp;&nbsp; | &nbsp;&nbsp; <b>S5P latest available:</b> {latest_label} <br> <span class="muted"> Dashboard refresh is not the same as satellite acquisition. A refreshed page can show the same satellite observation until a new scene becomes available. </span> </div> """,
    unsafe_allow_html=True,
)
...
with tabs[0]:
    ...
    st.markdown(
        """ <div class="card"> <b>Fusion logic:</b> methane + thermal + deformation + pressure proxy + moisture + wind + local spatial clustering. The score is an internal screening index, not a regulatory risk probability. </div> """,
        unsafe_allow_html=True,
    )
...
with tabs[1]:
    ...
    st.markdown(
        """ <div class="card"> <b>Interpretation:</b> A high atmospheric CH₄ value near a landfill is a screening signal. It does not by itself prove source attribution or provide a tonnes/hour emission rate. Atmospheric transport, background methane and other sources must be considered. </div> """,
        unsafe_allow_html=True,
    )
...
with tabs[2]:
    ...
    st.markdown(
        """ <div class="card"> <b>Physics used in the notebook concept:</b> Fourier heat conduction, thermal transport and first-order reaction/decay concepts. In this implementation, satellite/CSV thermal values are treated as observations or inputs; the app does not claim to directly measure a subsurface fire from surface temperature alone. </div> """,
        unsafe_allow_html=True,
    )
...
with tabs[5]:
    ...
    for name, formula, meaning in eqs:
        st.markdown(
            f""" <div class="card"> <b>{name}</b><br> <div style="font-size:1.25rem">{formula}</div> <span class="muted">{meaning}</span> </div> """,
            unsafe_allow_html=True,
        )
...
with tabs[7]:
    ...
    for _, r in ranked.head(12).iterrows():
        ...
        st.markdown(
            f""" <div class="card"> <div style="font-size:1.1rem;font-weight:800"> {risk_symbol(label)} {r['name']} <span class="{cls}" style="float:right"> {label} — {r['risk_score']:.1f}/100 </span> </div> <div class="muted"> CH₄ {r['methane_ppb']:.0f} ppb • thermal ΔT {r['thermal_anomaly_c']:.1f} °C • deformation {r['deformation_mm_yr']:.2f} mm/yr • pressure proxy {r['pressure_psi']:.1f} psi </div> </div> """,
            unsafe_allow_html=True,
        )
...
with tabs[8]:
    ...
    st.markdown(
        """ <div class="card"> <b>Anti-hallucination rule:</b> ZeroWaste.AI should never label a proxy as a satellite measurement. Missing sensor layers stay marked as unavailable. </div> """,
        unsafe_allow_html=True,
    )
...
st.markdown(
    """ <div class="tiny"> ZeroWaste.AI concept layers represented in this build include multi-sensor fusion, physics-informed calculations, plume transport, thermal screening, deformation / effective-stress analysis, spatial clustering, long-horizon scenarios and an operator-prioritisation interface. The notebook notes describe these as a multi-physics architecture rather than a single satellite product. </div> """,
    unsafe_allow_html=True,
)